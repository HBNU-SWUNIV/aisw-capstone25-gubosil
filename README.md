...
- ### 필요성
  - **사이버 위협의 증가**: 해킹 및 사이버 공격이 지속적으로 급증하면서 개인·기업·공공기관을 가리지 않고 피해가 확산되고 있으며, 사회 전반의 정보 인프라를 위협한다.
  - **암호화 트래픽의 한계**: 현대 네트워크 트래픽이 대부분 암호화(HTTPS, VPN 등)되어 있어 기존의 패킷 기반 분석으로는 이상 징후를 탐지하기 어렵다.
  - **지능형 대응의 필요성**: 패킷 내용이 아닌 트래픽의 흐름(flow) 패턴을 기반으로 학습하는 AI/딥러닝 모델 및 실시간으로 탐지하는 지능형 시스템 구현이 필요하다.

- ### 기존 해결책의 문제점
  - **패킷 기반 분석 한계**:
기존의 패킷 기반 분석 방식은 트래픽의 대부분이 암호화(HTTPS, VPN 등)되어 있어 내부 데이터를 직접 확인하기 어렵다.
그로 인해 공격 여부를 정확하게 식별하기 힘들고 특히 암호화된 환경에서는 탐지 성능이 크게 저하되는 한계가 있다.

  - **복호화 기반 접근의 한계**:
암호화된 트래픽을 복호화해 분석하는 방식은 개인정보 침해 위험이 있을 뿐 아니라 복호화 과정에서 높은 연산 부담이 발생해 시스템 성능 저하로 이어진다. 특히 대규모 네트워크 환경에서는 실시간 탐지가 어려워 실질적인 적용에 제한적이다.

  - **배치 처리의 한계**: 일정 시간동안 데이터를 모아서 분석하는 경우, 이상 징후가 발생한 시점과 이를 인지하는 시점 사이에 큰 시간 지연이 발생하여 즉각적인 대응이 불가능하다.
 
&rArr; 본 시스템은 이러한 문제점을 해결하기 위해 **딥러닝 모델**을 활용하여 **트래픽 흐름 패턴**을 **실시간**으로 분석한다.
...
## Case Study
  ...
  - ### Description
    - ### 실험 환경 구성
      - 두 대의 라즈베리파이를 사용하여 하나는 트래픽을 생성하는 송신 장치로, 다른 하나는 AI 기반 이상 탐지 시스템으로 구성
        
          > 라즈베리파이는 실제 IoT 환경을 가정하여 저전력·소형 하드웨어에서의 실시간 동작 가능성을 검증하기 위해 사용
      ...
      - **서버 라즈베리파이**: 랜덤 트래픽 데이터를 정하고, 모니터의 버튼을 눌러 클라이언트 라즈베리파이에 데이터를 전송
        ...
        <br>
      - **클라이언트 라즈베리파이**: AI 모델을 통해 실시간으로 이상 트래픽과 정상 트래픽을 탐지하고 모니터에 알림을 띄움
...
## Conclusion
  ...
  - ### 연구결과
    ...
    - 비정상적인 트래픽을 실시간으로 탐지하고 대응할 수 있는 GUI를 개발하여 관리자와 사용자가 직관적으로 이상 트래픽 상태를 인식하고 대응할 수 있는 환경을 제공한다.
